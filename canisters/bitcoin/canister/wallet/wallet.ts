...
import * as ecdsaApi from "../ecdsa/ecdsa";

/// Returns the P2PKH address of this canister at the given derivation path.
export async function get_p2pkh_address(
...
export async function bitcoin_transfer(
  ...
  const ownAddressString = public_key_to_p2pkh_address(network, ownPublicKey);

  console.log("Fetching UTXOs...");
  const ownUtxos = (await bitcoinApi.get_utxos(network, ownAddressString))
    .utxos;
  ...
  const transaction = await build_transaction(
    ownPublicKey,
    ownAddress,
    ownUtxos,
    dstAddress,
  ...
  await bitcoinApi.send_transaction(network, signedTransactionBytes);
  console.log("Done");
...
// destination address.
async function build_transaction(
  ownPublicKey: blob,
...
function build_transaction_with_fee(
  ...
  if (remainingAmount >= DUST_THRESHOLD) {
    outputs.push(BitcoinTxOut.new(remainingAmount, ownAddress.script_pubkey()));
  }
...
async function sign_transaction(
  ...
  for (let index = 0; index < transaction.inputs.length; index++) {
    ...
    const signature = await signer(keyName, derivationPath, sighash.to_vec());
...
function public_key_to_p2pkh_address(
  network: BitcoinNetwork,
  publicKey: blob
): string {
  // sha256 + ripmd160
